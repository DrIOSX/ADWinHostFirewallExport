...
Describe Get-Something {
    BeforeAll {
        Mock -CommandName Resolve-FirewallResourceString -MockWith { $PrivateData }
"
    }
    Context 'Return values' {
        ...
        It 'Returns a string from Resolve-FirewallResourceString' {
            Assert-MockCalled Resolve-FirewallResourceString -Times 1 -Exactly -Scope It
    ...
    Context 'Pipeline' {
        It 'Accepts values from the pipeline by value' {
            $return = 'value1', 'value2' | Get-Something
                Assert-MockCalled Resolve-FirewallResourceString -Times 2 -Exactly -Scope It
    ...
    Context 'ShouldProcess' {
        ...
        It 'Does not call Resolve-FirewallResourceString if WhatIf is set' {
            $return = Get-Something -Data 'value' -WhatIf
            $return | Should -BeNullOrEmpty
            Assert-MockCalled Resolve-FirewallResourceString -Times 0 -Scope It
        }
    }
}
